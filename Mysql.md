...
5. 索引
   ...
   - 为什么索引会降低增删改的速度
     ...
     因为DML操作时，如果有索引，会重新构建**索引**树，消耗数据库性能
   ...
   - 常问到的几个概念
     ...
     - 索引下推
       ...
       对于复合索引来说，如果没有索引下推，则会先根据第一列拿到全部数据，然后在server进行过滤，有索引下推之后就会在存储层直接查询出数据。
   ...
   - sql优化的技巧：
     ...
     a.like模糊查询 左侧加% 用不到索引，右侧可以用到
     b.name is null 也可以使用到索引
     c.优化器会判断使用索引和全表扫描的效率 比较那个效率高使用那个
     d.or语句 前边有索引后边没有索引 索引会失效
     e.字符串类型的列要加引号 否则不会用到索引
     f.能用bottowen and 不用 in 或者直接用 exists
     g.分组查询是 默认会自动进行排序 order by null
...
6. 锁
   ...
   - 行锁、表锁的区别
     ...
     - 行锁
       ...
       开销小，加锁快，不会产生死锁，锁粒度大，容易产生冲突，并发度低
...
7. 事务
    ...
    - 事务的ACID特性 
      ...
      - 隔离性：事务的执行不受其他事务的干扰，当数据库被多个客户端并发访问时。隔离他们的操作，防止出现脏读、幻读、不可重复读。
    ...
    - 事务的隔离级别 SQL92标准
      ...
      mysql 默认的事务隔离级别是 repeatable read，其实在这种隔离级别下其实就已经解决了大部分的问题
